...
更新前请先从系统托盘彻底退出旧版 Dotii 管理中心。建议把新版 ZIP 完整解压到一个新的空文件夹，直接运行新版 `DotiiManagementCenter.exe`；确认新版正常后，再删除旧版的解压文件夹。不要把新版直接覆盖到旧目录，以免残留文件造成版本混用。

运行配置保存在 `%LOCALAPPDATA%\StateDisplay`，删除旧版程序文件夹不会清除现有设置。若新版本包含 Dotii 固件更新，请用 USB 连接设备，并在新版管理中心中重新执行“一键烧录”。
